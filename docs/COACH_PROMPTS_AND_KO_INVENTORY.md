...
2. [`server/prompts/` 모듈별](#2-serverprompts-모듈별--프롬프트문구-전문--용도) — 프롬프트·문구 전문 + 용도  
...
#### `buildTomorrowPlanCollabSystemBlock(focus, context)` — `focus === "life"` 일 때 (전문, `${ctxJson}` 치환)
...
#### `buildTomorrowPlanCollabSystemBlock` — 그 외 (`study` 등) (전문)
...
#### `tomorrowPracticeSynthesizeSystem` (전문)
...
#### `buildTomorrowPlanBooksSynthesizeSystem(bookIdsJson)` (전문)
...
#### `wrapSituationJsonForSynthesize(context)` (전문)
